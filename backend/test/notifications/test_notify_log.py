...
def test_write_to_file():
  ...
  with patch("project.notifications.notify_log.open", open_mock, create=True):
      notify = NotifyLog("message")
      notify.write_to_file("test.txt", notify.message)
  open_mock.assert_called_with("test.txt", "a")
  open_mock.return_value.write.assert_called_once_with(notify.message)

def test_send(tmp_path):
  notify = NotifyLog("message", str(tmp_path))
  sent = notify.send()
  assert sent == True
